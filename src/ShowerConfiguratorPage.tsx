...
const SUBTYPE_IMG = {
  ...
  "paravan-tavan": null,
  "fix-batant-standard": "/fix cu usa batanta.png",
  "fix-batant-1l": "/fix + usa batanta cu o latura.png",
      ...
      <main className="configurator-grid" style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px", display: "grid", gridTemplateColumns: "1fr 340px", gap: 24 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <SectionCard num={subtypes.length > 1 ? "06" : "05"} label="Opțiuni & Accesorii">
            ...
            <ToggleOption checked={inclManerRect} onChange={v => setConfig(c => ({ ...c, inclManerRect: v, inclManerScoica: false }))} label={p.options?.manerRectangular?.name || "Mâner Rectangular"} desc={p.options?.manerRectangular?.desc} price={`${p.options?.manerRectangular?.price || 80}€`} />
          </SectionCard>
        </div>
